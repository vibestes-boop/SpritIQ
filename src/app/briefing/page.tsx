...
import { Brain, TrendingDown, TrendingUp, Minus, RefreshCw, Sparkles, WifiOff, Clock } from "lucide-react";
  ...
  return (
    <main className="min-h-dvh flex flex-col">
      ...
      <header className="flex items-center justify-between px-4 pt-4 pb-2">
        ...
        <button
          onClick={refresh}
          style={{ width: "36px", height: "36px", borderRadius: "10px", background: "#111118", border: "1px solid #1E1E2E", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer" }}
      ...
      <div className="flex-1 flex flex-col gap-4 px-4 py-2 animate-fade-in">
        ...
        {!loading && data?.source === "demo" && (
          <div style={{
            ...
            <p style={{ fontSize: "12px", color: "#F59E0B" }}>Demo-Modus — RSS-Nachrichten nicht gefunden oder Qwen nicht erreichbar</p>
          </div>
        )}
        ...
        {!loading && data?.source === "ai" && (
          <div style={{
            ...
            <p style={{ fontSize: "12px", color: "#22C55E" }}>Echte KI-Analyse mit Qwen AI · {data.articlesAnalyzed} Artikel aus {new Set(data.newsItems.map(n => n.source)).size} Quellen</p>
          </div>
        )}
            ...
            <div className="flex flex-col gap-3">
              <div className="flex items-center gap-3">
                <div style={{
                  ...
                  background: `rgba(${data.recommendation === "TANKEN EMPFOHLEN" ? "34,197,94" : data.recommendation === "NICHT TANKEN" ? "239,68,68" : "245,158,11"},0.12)`,
                  border: `1px solid rgba(${data.recommendation === "TANKEN EMPFOHLEN" ? "34,197,94" : data.recommendation === "NICHT TANKEN" ? "239,68,68" : "245,158,11"},0.25)`,
                ...
                <div>
                  ...
                  <p style={{ fontFamily: "'Space Grotesk', sans-serif", fontSize: "18px", fontWeight: 700, color: recColor }}>
                    {data.recommendation}
                  </p>
                </div>
              </div>
        ...
        <div style={{ display: "flex", gap: "8px" }}>
          ...
          <Card variant="flat" style={{ flex: 1, padding: "14px" }}>
            ...
            {loading ? <SkeletonBlock h="24px" w="60%" /> : (
              <div className="flex items-center gap-2">
                <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: "22px", fontWeight: 700, color: "#F8FAFC" }}>
                  {data?.confidence ?? "–"}
                </span>
        ...
        {(loading || data?.politicalContext) && (
          <Card variant="flat" style={{ padding: "14px 16px" }}>
            <p style={{ fontSize: "11px", color: "#3B82F6", fontWeight: 600, letterSpacing: "0.05em", marginBottom: "6px" }}>POLITIK</p>
            {loading ? (
              <div className="flex flex-col gap-2">
                <SkeletonBlock h="13px" />
                <SkeletonBlock w="70%" h="13px" />
              </div>
            ) : (
          ...
          <div className="flex flex-col gap-2">
            ...
            {!loading && (!data?.newsItems || data.newsItems.length === 0) && (
              <p style={{ fontSize: "13px", color: "#64748B", textAlign: "center", padding: "16px" }}>
                Keine relevanten Nachrichten gefunden
              </p>
            )}
          </div>
        </div>
