...
function getRecommendation(stations: Station[], fuelType: FuelType) {
  if (!stations.length)
    return { status: "medium" as const, reason: "Preise werden geladen..." };
  const prices = stations
    .map((s) => s[fuelType] as number | false)
    .filter((p): p is number => typeof p === "number" && p > 0);
  if (!prices.length)
    return { status: "medium" as const, reason: "Keine Preisdaten verfügbar." };
...
export default function HomePage() {
  ...
  const noFuelHint =
    !loading && stations.length > 0 && effectiveFuelType !== fuelType;
    ...
    for (const s of stations) {
      for (const ft of ["e10", "e5", "diesel"] as FuelType[]) {
        ...
        if (typeof p === "number" && p > 0) {
          if (prices[ft] === null || p < prices[ft]!) prices[ft] = p;
        }
      }
    }
    checkAlarms(prices);
      ...
      <main className="min-h-dvh flex flex-col">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              style={{
                ...
                height: "36px",
                borderRadius: "10px",
                background: showAlarmPanel
                  ? "rgba(245,158,11,0.12)"
                  : "#111118",
                border: `1px solid ${showAlarmPanel ? "rgba(245,158,11,0.3)" : "#1E1E2E"}`,
                display: "flex",
                alignItems: "center",
            ...
            >
              <Bell
                size={16}
                color={alarms.some((a) => a.enabled) ? "#F59E0B" : "#64748B"}
              />
        ...
        <div className="flex-1 flex flex-col gap-4 px-4 py-2 animate-fade-in">
          ...
          {showAlarmPanel && (
            <Card variant="flat" style={{ padding: "16px" }}>
              ...
              >
                ...
                >
                  <span style={{ fontSize: "12px", color: "#64748B" }}>
                    {fuelType.toUpperCase()} unter
                  </span>
                  <button
                    onClick={() =>
                      setAlarmThreshold((v) =>
                        Math.max(0.5, +(v - 0.01).toFixed(2)),
                      )
                    }
                    style={{
                      background: "none",
                ...
                <Button
                  size="sm"
                  variant="primary"
                  onClick={() => addAlarm(fuelType, alarmThreshold)}
                >
                  +
                </Button>
              </div>
              {alarms.length === 0 ? (
                ...
                >
                  Noch keine Alarme — erstelle deinen ersten!
                </p>
              ) : (
                <div
                  style={{
          ...
          <Card variant="flat" style={{ padding: "14px 16px" }}>
            <div className="flex items-start gap-3">
              ...
              >
                <Clock
                  size={16}
                  color={bestTime.recommendNow ? "#22C55E" : "#EF4444"}
                />
              </div>
            ...
            {recentHistory.length >= 2 ? (
              <div style={{ marginTop: "12px" }}>
                ...
                >
                  {fuelType.toUpperCase()} Verlauf · {recentHistory.length}{" "}
                  Messpunkte
                </p>
                <PriceChart
            ...
            {[
              ...
              <button
                ...
                style={{
                  ...
                  padding: "8px",
                  borderRadius: "10px",
                  border: `1px solid ${sortMode === key ? "#22C55E" : "#1E1E2E"}`,
                  background:
                    sortMode === key ? "rgba(34,197,94,0.08)" : "#111118",
                  color: sortMode === key ? "#22C55E" : "#64748B",
                  fontSize: "12px",
                  fontWeight: 600,
                  cursor: "pointer",
          ...
          {!loading && allPrices.length > 0 && (
            ...
            >
              {[
                {
                  label: "Günstigster",
                  val: Math.min(...allPrices),
                  color: "#22C55E",
                },
                {
                  label: "Durchschnitt",
                  val: allPrices.reduce((a, b) => a + b, 0) / allPrices.length,
                  color: "#F59E0B",
                },
                {
                  label: "Teuerster",
                  val: Math.max(...allPrices),
                  color: "#EF4444",
                },
              ].map(({ label, val, color }) => (
                <div
          ...
          <div style={{ marginLeft: "-16px", marginRight: "-16px" }}>
            ...
            {loading ? (
              <div className="flex flex-col" style={{ gap: 0 }}>{[0, 1, 2].map((i) => <StationSkeleton key={i} />)}</div>
