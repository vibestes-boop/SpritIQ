...
export default function DetourModal({
  ...
  const ResultIcon = result.worthIt
    ? TrendingDown
    : result.netSavings > 0
      ? Minus
      : TrendingUp;

  const resultColor = result.worthIt
    ? "#22C55E"
    : result.netSavings > 0
      ? "#F59E0B"
      : "#EF4444";
    ...
    >
      ...
      >
        ...
        <div style={{ marginBottom: "24px" }}>
          ...
          <div className="flex justify-between mt-1">
            {["0.5", "5", "10", "15", "20"].map((v) => (
              <span key={v} style={{ fontSize: "10px", color: "#64748B" }}>
                {v}km
              </span>
            ))}
          </div>
        ...
        >
          ...
          <span style={{ fontSize: "12px", color: "#64748B" }}>
            {profile.consumption}L/100km
          </span>
          <span style={{ fontSize: "12px", color: "#64748B" }}>·</span>
          <span style={{ fontSize: "12px", color: "#64748B" }}>
            {profile.tankSize}L Tank
          </span>
        </div>
        ...
        >
          ...
          <div style={{ display: "flex", gap: "8px", marginTop: "16px" }}>
            ...
            <div style={{ flex: 1, textAlign: "center" }}>
              ...
              >
                {result.netSavings >= 0 ? "+" : ""}
                {result.netSavings.toFixed(2).replace(".", ",")}€
              </p>
            </div>
          </div>
        </div>
